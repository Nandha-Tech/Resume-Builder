      ...
      <Grid size={{ xs: 12, md: 7 }}>
        <Paper sx={{ p: 2, borderRadius: 3, boxShadow: 2, mb: 3 }}>
          ...
          <Stack direction="row" spacing={1} alignItems="center" sx={{ mb: 1 }}>
            <TextField size="small" label="Add a skill" onKeyDown={(e) => {
              if (e.key === 'Enter') {
                e.preventDefault()
            ...
            <Button variant="contained" onClick={() => {
              const input = document.querySelector('#skill-input') as HTMLInputElement | null
              if (input) { addSkill(input.value); input.value='' }
            }}>Add</Button>
          </Stack>
          <Stack direction="row" spacing={1} useFlexGap flexWrap="wrap">
            {data.skills.map((s, i) => (
              <Chip key={i} label={s} onDelete={() => setData(d => ({...d, skills: d.skills.filter(x => x!==s)}))} />
            ))}
          </Stack>
        </Paper>
